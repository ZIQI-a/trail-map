import { AppstoreAddOutlined, CarOutlined, ClockCircleOutlined, EnvironmentOutlined, FlagOutlined, RightOutlined, UnorderedListOutlined } from '@ant-design/icons';
import { Alert, Badge, Button, Empty, Input, Popover, Segmented, Select } from 'antd';
...
export function TripPlannerDock({
  ...
  const totalDistanceText = planResult ? formatRouteDistance(planResult.totalDistanceMeters) : '--';
  const totalTravelText = planResult ? formatRouteDuration(planResult.totalTravelDurationSeconds) : '--';
  const totalTripText = planResult ? formatTripDuration(planResult.totalTripDurationMinutes) : '--';
  // 行程池通过 Popover 展开，避免长期占用地图视野。
  const tripPopoverContent =
    tripSpots.length === 0 ? (
      <Empty className={styles.popoverEmpty} image={Empty.PRESENTED_IMAGE_SIMPLE} description="先从右侧详情把景点加入行程" />
    ) : (
      <div className={styles.tripPopoverList}>
        {tripSpots.map((spot, index) => (
          <div className={styles.tripPopoverItem} key={spot.id}>
            ...
            <button className={styles.removeButton} type="button" aria-label={`删除${spot.name}`} onClick={() => onRemoveSpot(spot.id)}>
              ×
            </button>
          </div>
        ))}
  ...
  return (
    <section className={styles.tripDock} aria-label="行程规划区域">
      <div className={styles.tripCluster}>
        <Popover placement="topLeft" trigger="click" content={tripPopoverContent} overlayClassName={styles.tripPopover}>
          <button className={styles.tripLauncher} type="button">
            ...
            <div className={styles.tripLauncherText}>
              <strong>我的行程</strong>
              <span>{tripSpots.length === 0 ? '还没有加入景点' : `已选 ${tripSpots.length} 个景点`}</span>
            </div>
      ...
      <div className={styles.plannerRail}>
        <label className={`${styles.railCard} ${styles.startCard}`}>
          <span>起点</span>
          <Input prefix={<EnvironmentOutlined />} value={startPoint} placeholder="例如：春熙路地铁站" onChange={(event) => onStartPointChange(event.target.value)} />
        </label>

        <label className={styles.railCard}>
          <span>交通方式</span>
          <Select value={selectedTransport} options={transportTypes} onChange={(value) => onTransportChange(value as TransportType)} />
        </label>

        <label className={styles.railCard}>
          <span>规划模式</span>
          <Segmented className={styles.modeSegmented} value={selectedPlanMode} options={planModes} onChange={(value) => onPlanModeChange(value as PlanMode)} />
        </label>
        ...
        <Button className={styles.planButton} type="primary" disabled={tripSpots.length === 0} loading={planning} onClick={onPlanRoute}>
      ...
      {planError ? (
        <div className={styles.feedbackRow}>
          <Alert type="error" showIcon message="行程规划失败" description={planError} />
        </div>
      ) : null}
    </section>
  );
