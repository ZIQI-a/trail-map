  ...
  return (
    <PersonalPageLayout currentUser={currentUser} onLogout={handleLogout}>
      ...
      <div className={styles.dashboardGrid}>
        ...
        <section className={styles.cardBox}>
          <header className={styles.cardHeader}>
            ...
            <span className={styles.viewAll} onClick={() => navigate("/trips")}>
              查看全部 <RightOutlined />
            </span>
          </header>
          {userTrips.length === 0 ? (
              ...
              {checkinSpots.slice(0, 4).map((spot) => (
                <div key={spot.checkinId} className={styles.checkinItem}>
                  ...
                  <img
                    ...
                    src={spot.coverUrl || undefined}
                    alt={spot.name}
                  />
                  <div className={styles.checkinDetail}>
                    <h5>{spot.name}</h5>
                    <span>
                      {spot.cityName} · {formatDate(spot.checkedInAt)}
                    </span>
                  </div>
                </div>
              ))}
      ...
      <section className={styles.favoritesSection}>
        <header className={styles.cardHeader}>
          <h3>
            <HeartFilled style={{ color: "#f87171" }} /> 我的收藏
          </h3>
          <span
            className={styles.viewAll}
        ...
        <div className={styles.favGrid}>
          {favoriteSpots.map((spot) => (
            <div
              ...
              onClick={() => navigate(`/?spotId=${spot.spotId}`)}
            >
              <img
