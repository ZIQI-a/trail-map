...
| `clearAuthToken()` | 清除 localStorage 中的 token，并通知当前页面更新登录状态 |
| `subscribeAuthToken()` | 订阅当前页面及跨标签页的 Token 变化 |

页面通过 `useAuthToken()` 响应式读取登录状态；任一接口返回 401 时，请求层会清除过期 Token。地图、城市和景点等公开内容随后按匿名用户继续展示。
...
4. **登录失效处理**：收到 401 后统一清除 Token 并刷新页面认证状态
5. **环境变量配置**：通过 `VITE_API_BASE_URL` 支持不同环境的 API 地址
